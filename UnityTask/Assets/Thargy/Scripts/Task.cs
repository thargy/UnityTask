...
namespace Apocraphy.Assets.Scripts.Threading
{
    ...
    public class Task : ITask
    {
        ...
        /// <remarks><para>This overload should only be used for very quick tasks, ideally you should accept and regularly check
        /// a cancellation token.</para></remarks>
        ...
        /// <returns>The task result.</returns>
        /// <remarks><para>During long running operations, checking <paramref name="cancellationToken"/> allows the task to voluntarily execute
        /// as soon as possible and free resources. This should be done relatively frequently.</para></remarks>
        public delegate void TaskWithCancellationDelegate(CancellationToken cancellationToken);


        public delegate void FailedTaskDelegate(Exception exception);

        public delegate void FailedTaskWithCancellationDelegate(Exception exception, CancellationToken cancellationToken);

        /// <summary>
        /// All active tasks.
        /// </summary>
        private static OrderedDictionary _tasks = new OrderedDictionary();

        /// <summary>
        /// Gets all active tasks (in <see cref="Id"/> order).
        /// </summary>
        /// <value>The active tasks.</value>
        /// <remarks><para>These are all tasks where the <see cref="State"/> is <see cref="TaskStatus.Running">running or less</see>.</para></remarks>
        public static IEnumerable<Task> Tasks { get { return _tasks.Values.Cast<Task>().Where(t => t.State <= TaskStatus.Running); } }
        ...
        public static Task Cancelled = new Task(
            (TaskDelegate)null,
        ...
        {
            return new Task((TaskDelegate)null, TaskManager.Immediate, token).SetResult(null);
        }

        public static Task FromException(Exception excetpion, CancellationToken token = default(CancellationToken))
        {
            return new Task((TaskDelegate)null, TaskManager.Immediate, CancellationToken.None).SetException(null);
        }

        public static Task<T> FromException<T>(Exception excetpion, CancellationToken token = default(CancellationToken))
        {
            return
                new Task<T>((Task<T>.ResultTaskDelegate) null, TaskManager.Immediate, CancellationToken.None).
                    SetException(null);
        }
        ...
        /// <param name="scheduler">The scheduler.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <remarks><para>For internal use only, you <i>must</i> set the <see cref="Action"/> property during construction.</para></remarks>
        public Task(
            ITaskScheduler scheduler = null,
            CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        public TaskStatus State
        {
            get
            {
                ...
                if (Token.IsCancellationRequested && _state != (int)TaskStatus.Cancelled)
        ...
        protected bool ChangeState(TaskStatus nextState)
        {
            ...
            if (!Token.IsCancellationRequested && nextState != TaskStatus.Cancelled)
            {
                    ...
                    switch (nextState)
                    {
                        ...
                        default:
                            throw new ArgumentOutOfRangeException(
                                "nextState",
                                nextState,
                                string.Format("Cannot transition to the '{0}' state as there is no action defined.", nextState));
                    }
                else
                    switch (nextState)
                    {
        ...
        public ITask<TResult> OnSuccess<TResult>(Task<TResult>.ResultTaskDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult>)OnSuccess(new Task<TResult>(function, scheduler, Token));
        }

        public ITask<TResult> OnSuccess<TResult>(Task<TResult>.ResultTaskWithCancellationDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult>)OnSuccess(new Task<TResult>(function, scheduler, Token));
        }

        public ITask<TResult, TProgress> OnSuccess<TResult, TProgress>(Task<TResult, TProgress>.ProgressTaskDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult, TProgress>)OnSuccess(new Task<TResult, TProgress>(function, scheduler, Token));
        }

        public ITask<TResult, TProgress> OnSuccess<TResult, TProgress>(Task<TResult, TProgress>.ProgressTaskWithCancellationDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult, TProgress>)OnSuccess(new Task<TResult, TProgress>(function, scheduler, Token));
        }
        ...
        public ITask<TResult> OnFailure<TResult>(Task<TResult>.FailedResultTaskDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult>)OnFailure(new Task<TResult>(() => function(Exception), scheduler, Token));
        }

        public ITask<TResult> OnFailure<TResult>(Task<TResult>.FailedResultTaskWithCancellationDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult>)OnFailure(new Task<TResult>(t => function(Exception, t), scheduler, Token));
        }

        public ITask<TResult, TProgress> OnFailure<TResult, TProgress>(Task<TResult, TProgress>.FailedProgressTaskDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult, TProgress>)OnFailure(new Task<TResult, TProgress>(n => function(Exception, n), scheduler, Token));
        }

        public ITask<TResult, TProgress> OnFailure<TResult, TProgress>(Task<TResult, TProgress>.FailedProgressTaskWithCancellationDelegate function, ITaskScheduler scheduler = null)
        {
            return (ITask<TResult, TProgress>)OnFailure(new Task<TResult, TProgress>((n, t) => function(Exception, n, t), scheduler, Token));
        }
    ...
    public class Task<TResult> : Task, ITask<TResult>
    {
        ...
        public delegate TResult FailedResultTaskWithCancellationDelegate(Exception exception, CancellationToken cancellationToken);

        public delegate void ContinuationTaskDelegate(TResult result);

        public delegate void ContinuationTaskWithCancellationDelegate(TResult result, CancellationToken cancellationToken);

        public delegate T ContinuationResultTaskDelegate<out T>(TResult result);

        public delegate T ContinuationResultTaskWithCancellationDelegate<out T>(TResult result, CancellationToken cancellationToken);

        public Task(ResultTaskDelegate function, ITaskScheduler scheduler = null, CancellationToken cancellationToken = default(CancellationToken)) : base(scheduler, cancellationToken)
        {
            if (function != null)
                Action = () =>
                {
        ...
        public Task(ResultTaskWithCancellationDelegate function, ITaskScheduler scheduler = null, CancellationToken cancellationToken = default(CancellationToken)) : base(scheduler, cancellationToken)
        {
            if (function != null)
                Action = () =>
                {
        ...
        public Task(ITaskScheduler scheduler = null, CancellationToken cancellationToken = default(CancellationToken)) : base(scheduler, cancellationToken)
        {
        }
        ...
        {
            return (ITask<TResult>)OnSuccess(new Task<TResult>(() => Result, TaskManager.AfterDelay(delay, TaskManager.Immediate), Token));
        }
        ...
        {
            return (ITask<TResult>)OnSuccess(new Task<TResult>(() => Result, TaskManager.AfterDelay(millisecondsDelay, TaskManager.Immediate), Token));
        }
        ...
        public ITask<T> OnSuccess<T>(ContinuationResultTaskWithCancellationDelegate<T> function, ITaskScheduler scheduler = null)
        {
            return (ITask<T>)OnSuccess(new Task<T>(t => function(Result, t), scheduler, Token));
        }

        public ITask<T, TProgress> OnSuccess<T, TProgress>(Task<T, TProgress>.ContinuationProgressTaskDelegate<TResult> function, ITaskScheduler scheduler = null)
        {
            return (ITask<T, TProgress>)OnSuccess(new Task<T, TProgress>(n => function(Result, n), scheduler, Token));
        }

        public ITask<T, TProgress> OnSuccess<T, TProgress>(Task<T, TProgress>.ContinuationProgressTaskWithCancellationDelegate<TResult> function, ITaskScheduler scheduler = null)
        {
            return (ITask<T, TProgress>)OnSuccess(new Task<T, TProgress>((n, t) => function(Result, n, t), scheduler, Token));
        }
    }

    /// <summary>
    /// The <see cref="Task{TResult, TProgress}"/> allows for the creation of <see cref="Task">tasks</see> that can notify subscribers of progress, and ultimately
    ...
    public sealed class Task<TResult, TProgress> : Task<TResult>, ITask<TResult, TProgress>
    {
        ...
        /// </summary>
        private readonly Queue<KeyValuePair<OnProgressDelegate, ITaskScheduler>> _onProgress = new Queue<KeyValuePair<OnProgressDelegate, ITaskScheduler>>();
        ...
        /// <remarks><para>This can be called repeatedly as the task makes progress.</para></remarks>
        public delegate void NotifyProgressDelegate(TProgress progress);

        /// <summary>
        /// Delegate of method for creating a task the allows for updating subscribers of it's progress, and ultimately returns a result.
        /// </summary>
        /// <param name="notifyProgress">The notify progress method.</param>
        /// <returns>The task result.</returns>
        /// <remarks><para>Calling the <paramref name="notifyProgress"/> method will update subscribers of the task's current progress.
        /// It can be called repeatedly whilst the task is running.</para></remarks>
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>The task result.</returns>
        /// <remarks>Calling the <paramref name="notifyProgress" /> method will update subscribers of the task's current progress.
        /// It can be called repeatedly whilst the task is running.</remarks>
        public delegate TResult ProgressTaskWithCancellationDelegate(NotifyProgressDelegate notifyProgress, CancellationToken cancellationToken);

        public delegate TResult FailedProgressTaskDelegate(Exception exception, NotifyProgressDelegate notifyProgress);

        public delegate TResult FailedProgressTaskWithCancellationDelegate(Exception exception, NotifyProgressDelegate notifyProgress, CancellationToken cancellationToken);

        /// <summary>
        /// Delegate of method for creating a task that accepts the result of a previous task and allows for updating subscribers of it's progress,
        ...
        /// <remarks>Calling the <paramref name="notifyProgress" /> method will update subscribers of the task's current progress.
        /// It can be called repeatedly whilst the task is running.</remarks>
        public delegate TResult ContinuationProgressTaskWithCancellationDelegate<in T>(T result, NotifyProgressDelegate notifyProgress, CancellationToken cancellationToken);

        /// <summary>
        /// Delegate of method that is called when a task makes progress.
        /// </summary>
        /// <param name="progress">The progress.</param>
        /// <remarks><para>This may be called repeatedly as the task makes progress.</para>
        /// <para>Although the associaed <see cref="Task{TResult, TProgress}"/> has a <see cref="Task{TResult,TProgress}.LatestProgress"/>
        /// property, race conditions mean it may not match the supplied <paramref name="progress"/> property (which may be out of date).  Depending
        /// on the <see cref="ITaskScheduler">scheduler</see> specified when adding the handler it isn't impossible that the various invocations
        /// occur out of order, though it is unlikely.</para>
        /// <para>If order is absolutely critical, consider using a <see cref="TProgress"/> type that incorporates an order field.</para></remarks>
        ...
        /// on the <see cref="ITaskScheduler">scheduler</see> specified when adding the handler it isn't impossible that the various invocations
        /// occur out of order, though it is unlikely.</para>
        /// <para>If order is absolutely critical, consider using a <see cref="TProgress" /> type that incorporates an order field.</para>
        /// <para>The progress methods should endevour to be relatively quick and shouldn't be called when the <paramref name="cancellationToken"/>
        /// is <see cref="CancellationToken.IsCancellationRequested">cancelled</see>.  As such this overload should be used with caution.</para></remarks>
        ...
        /// <param name="scheduler">The scheduler.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public Task(ProgressTaskDelegate function, ITaskScheduler scheduler = null, CancellationToken cancellationToken = default(CancellationToken)) : base(scheduler, cancellationToken)
        {
            if (function != null)
                Action = () =>
                {
        ...
        /// <param name="scheduler">The scheduler.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public Task(ProgressTaskWithCancellationDelegate function, ITaskScheduler scheduler = null, CancellationToken cancellationToken = default(CancellationToken)) : base(scheduler, cancellationToken)
        {
            if (function != null)
                Action = () =>
                {
        ...
        public ITask<TResult, TProgress> OnProgress(OnProgressWithCancellationDelegate action, ITaskScheduler scheduler = null)
        {
